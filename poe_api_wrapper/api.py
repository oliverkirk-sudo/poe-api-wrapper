...
class PoeApi:
    ...
    def __init__(self, cookie: str, proxy: str = ''):
        ...
        if proxy:
            ...
            self.client = Client(headers=self.HEADERS, timeout=180, proxies=proxy)
    ...
    def connect_ws(self, timeout=20):
        ...
        ws = websocket.WebSocketApp(self.channel_url, header=self.HEADERS, on_message=self.on_message,
                                    on_open=self.on_ws_connect, on_error=self.on_ws_error, on_close=self.on_ws_close)
        ...
        try:
            ...
            for message_str in data["messages"]:
                ...
                if message_data['message_type'] == 'refetchChannel':
                    ...
                    self.connect_ws()
                    return
                if message_data["message_type"] != "subscriptionUpdate" or message_data["payload"][
    ...
    def get_chat_history(self, bot: str = None, count: int = None, interval: int = 50, cursor: str = None):
        ...
        if bot == None:
            response_json = self.send_request('gql_POST', 'ChatHistoryListPaginationQuery',
                                              {'count': interval, 'cursor': cursor})
            ...
            for key, value in BOTS_LIST.items():
                if model == value:
                    handle = key
                    break
            response_json = self.send_request('gql_POST', 'ChatHistoryFilteredListPaginationQuery',
                                              {'count': interval, 'handle': handle, 'cursor': cursor})
            if response_json['data'] == None and response_json["errors"]:
                raise ValueError(
    ...
    def send_message(self, bot: str, message: str, chatId: int = None, chatCode: str = None, file_path: list = [],
                     suggest_replies: bool = False, timeout: int = 20) -> dict:
        ...
        if (chatId == None and chatCode == None):
            try:
                variables = {"chatId": None, "bot": bot, "query": message, "shouldFetchChat": True,
                ...
                if bot not in self.current_thread:
                    self.current_thread[bot] = [{'chatId': chatId, 'chatCode': chatCode, 'id': message_data['id'],
                                                 'title': message_data['title']}]
                elif self.current_thread[bot] == []:
                    self.current_thread[bot] = [{'chatId': chatId, 'chatCode': chatCode, 'id': message_data['id'],
                                                 'title': message_data['title']}]
                else:
                    self.current_thread[bot].append({'chatId': chatId, 'chatCode': chatCode, 'id': message_data['id'],
                                                     'title': message_data['title']})
            ...
            variables = {'bot': bot, 'chatId': chatId, 'query': message, 'shouldFetchChat': False,
                         'source': {"sourceType": "chat_input", "chatInputMetadata": {"useVoiceRecord": False}},
                         "clientNonce": generate_nonce(), 'sdid': "", 'attachments': attachments}
        ...
        def get_suggestions(queue, chatCode: str = None, timeout: int = 5):
            ...
            while True:
                ...
                if hasSuggestedReplies and edges:
                    ...
                    break
            queue.put({'text': message["text"], 'response': '', 'suggestedReplies': suggestions, 'state': state,
        ...
        if suggest_replies:
            ...
            try:
                ...
                yield {'text': message["text"], 'response': '', 'suggestedReplies': [], 'state': None,
                       'chatCode': chatCode, 'chatId': chatId, 'title': title}
    ...
    def cancel_message(self, chunk: dict):
        ...
        variables = {"messageId": chunk["messageId"], "textLength": len(chunk["text"]),
                     "linkifiedTextLength": len(chunk["linkifiedText"])}
    ...
    def get_previous_messages(self, bot: str, chatId: int = None, chatCode: str = None, count: int = 50,
                              get_all: bool = False):
        bot = bot.lower().replace(' ', '')
        try:
    ...
    def create_bot(self, handle, prompt, display_name=None, base_model="chinchilla", description="", intro_message="",
                   ...
                   markdown_rendering=True, suggested_replies=False, private=False, temperature=None):
    ...
    def explore_bots(self, search: str = None, count: int = 50, explore_all: bool = False):
        ...
        while len(bots) < count or explore_all:
            if search == None:
                ...
                result = self.send_request("gql_POST", query_name,
                                           {"query": search, "entityType": "bot", "count": 50, "cursor": new_cursor})
    ...
    def import_chat(self, bot: str = "", shareCode: str = ""):
        ...
        response_json = self.send_request('gql_POST', 'ContinueChatCTAButton_continueChatFromPoeShare_Mutation',
                                          variables)
